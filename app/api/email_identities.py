...
async def generate_email_identities(
    request: EmailIdentityBatchRequest, x_operator_role: Annotated[str, Header()] = "operator"
):
...
async def email_identity_command(
    identity_id: UUID,
    request: EmailIdentityCommandRequest,
    x_operator_role: Annotated[str, Header()] = "operator",
    x_operator_id: Annotated[str, Header()] = "dashboard",
):
    _admin(x_operator_role)
    try:
        return EmailIdentityOut.model_validate(
            await command_identity(identity_id, request.action, x_operator_id[:120])
        )
