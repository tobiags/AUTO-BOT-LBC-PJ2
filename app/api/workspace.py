...
async def _get_workspace(db):
    ...
    if workspace:
        ...
        user_count = (await db.execute(select(func.count()).select_from(User).where(User.workspace_id == workspace.id))).scalar() or 0
        if user_count == 1:
            sole_user = (await db.execute(select(User).where(User.workspace_id == workspace.id))).scalar_one()
...
async def create_user(
    ...
    async with get_db() as db:
        ...
        db.add(user)
        await db.flush()
        db.add(AuditEvent(actor=email, role=payload.role.value, action="workspace.user.create", target_type="user", target_id=str(user.id), input_summary={"email": email, "role": payload.role.value}, result_status="success"))
...
async def create_sector(
    ...
    async with get_db() as db:
        ...
        db.add(sector)
        await db.flush()
        db.add(AuditEvent(actor=x_operator_id[:100], role=x_operator_role, action="workspace.sector.create", target_type="sector", target_id=str(sector.id), input_summary={"name": payload.name, "region": payload.region, "department": payload.department}, result_status="success"))
        return sector
...
async def assign_sector_resources(
    ...
    async with get_db() as db:
        ...
        db.add(AuditEvent(actor=x_operator_id[:100], role=x_operator_role, action="workspace.sector.resources.update", target_type="sector", target_id=str(sector_id), input_summary={"accounts": len(payload.account_ids), "sims": len(payload.sim_ids), "proxies": len(payload.proxy_ids), "daily_limit_per_account": payload.daily_limit_per_account, "daily_limit_per_sim": payload.daily_limit_per_sim}, result_status="success"))
        return sector


@router.get("/sectors/{sector_id}/resources", response_model=SectorResourcesOut)
async def get_sector_resources(sector_id: UUID, x_operator_role: Annotated[str, Header()] = "operateur"):
    _require_role(x_operator_role, "operateur")
    async with get_db() as db:
        sector = await db.get(Sector, sector_id)
        if sector is None:
            raise HTTPException(404, detail={"code": "SECTOR_NOT_FOUND"})
        accounts = (await db.execute(select(SectorAccount).where(SectorAccount.sector_id == sector_id))).scalars().all()
        sims = (await db.execute(select(SectorSim).where(SectorSim.sector_id == sector_id))).scalars().all()
        proxies = (await db.execute(select(SectorProxy).where(SectorProxy.sector_id == sector_id))).scalars().all()
...
async def list_settings(x_operator_role: Annotated[str, Header()] = "operateur"):
    ...
    async with get_db() as db:
        workspace = await _get_workspace(db)
        return (await db.execute(select(WorkspaceSetting).where(WorkspaceSetting.workspace_id == workspace.id).order_by(WorkspaceSetting.key))).scalars().all()


@router.put("/settings", response_model=WorkspaceSettingOut)
async def upsert_setting(payload: WorkspaceSettingUpsert, x_operator_role: Annotated[str, Header()] = "operateur", x_operator_id: Annotated[str, Header()] = "dashboard"):
    _require_role(x_operator_role, "administrateur")
    async with get_db() as db:
        workspace = await _get_workspace(db)
        setting = (await db.execute(select(WorkspaceSetting).where(WorkspaceSetting.workspace_id == workspace.id, WorkspaceSetting.key == payload.key).with_for_update())).scalar_one_or_none()
        if setting is None:
            setting = WorkspaceSetting(workspace_id=workspace.id, key=payload.key, value=payload.value)
            db.add(setting)
        else:
            setting.value = payload.value
        setting.updated_by = x_operator_id[:100]
        await db.flush()
        db.add(AuditEvent(actor=x_operator_id[:100], role=x_operator_role, action="workspace.setting.update", target_type="setting", target_id=payload.key, input_summary={"key": payload.key}, result_status="success"))
        return setting
...
async def list_audit(limit: int = 100, x_operator_role: Annotated[str, Header()] = "operateur"):
    _require_role(x_operator_role, "operateur")
    async with get_db() as db:
        rows = (await db.execute(select(AuditEvent).order_by(AuditEvent.created_at.desc()).limit(min(limit, 500)))).scalars().all()
        return rows
